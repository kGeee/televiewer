...
export function parseVboExport(content: string): ParsedSession {
    ...
    for (const line of lines) {
        ...
        if (section === '[column names]') {
            ...
            // Keep as strings to detect 'N', 'S', 'E', 'W' tokens
            const values = trimmed.split(/\s+/);
            if (values.length >= columns.length) {
                dataRows.push(values);
            }
        } else if (inLapTiming && trimmed.startsWith('Start')) {
            const parts = trimmed.replace('Start', '').trim().split(/\s+/).map(v => parseFloat(v));
            if (parts.length >= 4) {
                startLine = {
    ...
    // Explicitly add critical columns
    ...
    if (colMap.velocity !== -1) {
        for (let i = 0; i < dataRows.length; i++) {
            const val = parseFloat(dataRows[i][colMap.velocity]);
            if (val > 10.0) { startIndex = Math.max(0, i - 20); break; }
        }
        for (let i = dataRows.length - 1; i >= 0; i--) {
            const val = parseFloat(dataRows[i][colMap.velocity]);
            if (val > 10.0) { endIndex = Math.min(dataRows.length - 1, i + 20); break; }
        }
    }
    ...
    let lastLapNum = 1;
    let prevDistance = 0;
    const initialTimeSeconds = colMap.time !== -1 && activeRows.length > 0 ? parseVboxTime(parseFloat(activeRows[0][colMap.time])) : 0;
    ...
    let validGpsCount = 0;
    let firstInvalidSample: { raw: [string, string], index: number } | null = null;

    activeRows.forEach((row, i) => {
        const rawTime = colMap.time !== -1 ? parseFloat(row[colMap.time]) : i * 0.1;
        const absTime = parseVboxTime(rawTime);
        const relTime = absTime - initialTimeSeconds;

        let isNewLap = false;
        if (colMap.lap !== -1) {
            const rowLap = parseFloat(row[colMap.lap]);
            if (rowLap !== lastLapNum) { isNewLap = true; lastLapNum = rowLap; }
        } else if (useGeometricSplitting && startLine && i > 0) {
            const currLat = parseCoord(parseFloat(row[colMap.lat]), true);
            const currLong = parseCoord(parseFloat(row[colMap.long]), false);
            const prevLat = parseCoord(parseFloat(activeRows[i - 1][colMap.lat]), true);
            const prevLong = parseCoord(parseFloat(activeRows[i - 1][colMap.long]), false);
        ...
        const speed = colMap.velocity !== -1 ? parseFloat(row[colMap.velocity]) : 0;
        const dt = i > 0 ? (parseVboxTime(colMap.time !== -1 ? parseFloat(activeRows[i][colMap.time]) : 0) - parseVboxTime(colMap.time !== -1 ? parseFloat(activeRows[i - 1][colMap.time]) : 0)) : 0.05;
        const distDelta = (speed / 3.6) * Math.max(0, dt);
        prevDistance += distDelta;

        // Parse GPS coordinates with validation
        const rawLatValue = colMap.lat !== -1 ? parseFloat(row[colMap.lat]) : 0;
        const rawLongValue = colMap.long !== -1 ? parseFloat(row[colMap.long]) : 0;

        let rawLat = colMap.lat !== -1 ? parseCoord(rawLatValue, true) : null;
        let rawLong = colMap.long !== -1 ? parseCoord(rawLongValue, false) : null;

        // HEMISPHERE CHECK (N/S/E/W)
        // Check next column for indicators if available
        if (colMap.lat !== -1 && row.length > colMap.lat + 1) {
            const nextToken = row[colMap.lat + 1].toUpperCase();
            if (nextToken === 'S') {
                if (rawLat !== null) rawLat = -Math.abs(rawLat);
            }
        }
        if (colMap.long !== -1 && row.length > colMap.long + 1) {
            const nextToken = row[colMap.long + 1].toUpperCase();
            if (nextToken === 'W') {
                if (rawLong !== null) rawLong = -Math.abs(rawLong);
            }
        }
        ...
        if (rawLat !== null && rawLong !== null) {
            ...
            if (!firstInvalidSample) {
                firstInvalidSample = { raw: [row[colMap.lat], row[colMap.long]], index: i };
            }
        }
        ...
        currentLapData.rpm.push(colMap.rpm !== -1 ? parseFloat(row[colMap.rpm]) : 0);
        currentLapData.throttle.push(colMap.throttle !== -1 ? parseFloat(row[colMap.throttle]) : 0);
        currentLapData.brake.push(colMap.brake !== -1 ? parseFloat(row[colMap.brake]) : 0);
        currentLapData.steering.push(colMap.steer !== -1 ? parseFloat(row[colMap.steer]) : 0);
        currentLapData.gear.push(colMap.gear !== -1 ? parseFloat(row[colMap.gear]) : 0);
        currentLapData.lat.push(finalLat);
        currentLapData.long.push(finalLong);
        currentLapData.avitime.push(colMap.avitime !== -1 ? parseFloat(row[colMap.avitime]) : 0);

        // Push extra columns
        extraColumnIndices.forEach(idx => {
            const key = columns[idx].toLowerCase();
            const val = parseFloat(row[idx]);
            currentLapData[key].push(isNaN(val) ? 0 : val);
        });
    });
